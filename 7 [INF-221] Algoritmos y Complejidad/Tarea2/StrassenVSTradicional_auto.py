...
entero = int(input("Ingrese la dimension de la matriz cuadrada: "))
entero1 =entero*entero*2
#filas = input("ingrese las filas de ambas matrices: ")
valores= ""
for x in range(entero1):
    val = int(random.randint(1,10))
    valores+=str(val)+" "
...
matriz_2 = []
vacia= []
divisor =  range(int(len(valores)/entero))
for x in divisor:
    if x < len(divisor)/2:
        vacia = valores[0:entero]
        matriz_1.append(vacia)
        valores = valores[entero:]
        vacia = []
    else : 
        vacia = valores[0:entero]
        matriz_2.append(vacia)
        valores = valores[entero:]
        vacia = []\
...
v2 = time.time()
print("Dimension de la matriz resultante: ", entero)
